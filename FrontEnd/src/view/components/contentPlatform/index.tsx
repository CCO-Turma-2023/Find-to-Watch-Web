...
export default function ContentPlatform() {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      const cardWidthPx =
        (isCurrentlySmall ? CARD_WIDTH_REM_SM : CARD_WIDTH_REM_MD) *
        rootFontSize;
      const cardGapPx =
        (isCurrentlySmall ? CARD_GAP_REM_SM : CARD_GAP_REM_MD) * rootFontSize;
      ...
      const newViewportWidth =
        finalVisibleCount * cardWidthPx +
        Math.max(0, finalVisibleCount - 1) * cardGapPx;
  ...
  return (
    <div
      ref={containerRef}
      className="flex w-full max-w-[91rem] justify-center"
    >
      <div className="flex items-center gap-2 md:gap-4">
        <motion.button
          className={`flex h-10 w-10 flex-shrink-0 cursor-pointer items-center justify-center rounded-full bg-[#1B1B1BE5] md:h-12 md:w-12 ${
            currentIndex <= 0 || platformLogos.length <= visibleCount
              ? "invisible"
              : ""
          }`}
          whileHover={{ scale: 1.05 }}
        ...
        <div
          ...
          style={{
            width: `${viewportWidth <= 328 ? viewportWidth : viewportWidth + 16}px`,
          }}
        >
          <motion.div
            className="flex gap-2 py-2 sm:ml-2 md:gap-4"
        ...
        <motion.button
          className={`flex h-10 w-10 flex-shrink-0 cursor-pointer items-center justify-center rounded-full bg-[#1B1B1BE5] md:h-12 md:w-12 ${
            currentIndex >= maxIndex || platformLogos.length <= visibleCount
              ? "invisible"
              : ""
          }`}
          whileHover={{ scale: 1.05 }}
